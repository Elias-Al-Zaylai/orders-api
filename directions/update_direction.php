...
try {
    ...
    /*
     * منع اختيار منفذ مشغول بمطلوب نشط آخر.
     */
    $busyExecutorStatement = $pdo->prepare("
        ...
        WHERE rd.executor_id = ?
          AND r.id <> ?
          AND r.status IN ('directed', 'received_by_executor', 'returned_to_executor')
        LIMIT 1
    ");

    $busyExecutorStatement->execute([
        $executorId,
        (int) $direction['requirement_id']
    ]);

    if ($busyExecutorStatement->fetchColumn()) {
