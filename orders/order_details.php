...
try {
    ...
    /*
     * التحقق أن المستخدم موجّه طلبات فعلًا،
     * وليس مجرد مستخدم عادي داخل نفس الإدارة.
     */
    $directionPermissionStatement = $pdo->prepare("
        SELECT 1
        FROM user_roles ur
    ...
    $directionPermissionStatement->execute([
        (int) $authUser['id']
    ]);

    $hasDirectionPermission =
        (bool) $directionPermissionStatement->fetchColumn();
